...
namespace Roleplay.Commands
{
    public class ChatIC
    {
        ...
        public void CMD_ame(IPlayer player, string mensagem) => Functions.SendMessageToNearbyPlayers(player, mensagem, TipoMensagemJogo.Ame, player.Dimension > 0 ? 7.5f : 20.0f);

        [Command("ado", "/ado (mensagem)", GreedyArg = true)]
        public void CMD_ado(IPlayer player, string mensagem) => Functions.SendMessageToNearbyPlayers(player, mensagem, TipoMensagemJogo.Ado, player.Dimension > 0 ? 7.5f : 20.0f);
    }
}