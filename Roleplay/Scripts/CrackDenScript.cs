...
namespace Roleplay.Scripts
{
    public class CrackDenScript : IScript
    {
        ...
        public async Task CrackDenSellItem(MyPlayer player, int id, int quantity)
        {
            ...
            {
                player.EmitStaffShowMessage($"Você não possui {quantity}x {Functions.GetEnumDisplay(item.ItemCategory)}.");
                return;
            }
            ...
            player.EmitStaffShowMessage($"Você vendeu {quantity}x {Functions.GetEnumDisplay(item.ItemCategory)} por ${value:N0}.", true);
            await player.GravarLog(LogType.Droga, $"Vender Boca de Fumo {item.CrackDenId} {item.ItemCategory} {quantity}", null);
        ...
        {
            await player.GravarLog(LogType.VisualizarVendasBocaFumo, id.ToString(), null);
            ...
            var sells = await context.CrackDensSells.Where(x => x.CrackDenId == id)
                ...
                .Select(x => new
                {
                    ...
                    Item = Functions.GetEnumDisplay(x.ItemCategory),
